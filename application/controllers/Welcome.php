...
class Welcome extends CI_Controller
{
	...
	public function index()
	{
		...
		$total = $this->db->get('peserta')->num_rows();
	...
	public function daftar()
	{
		...
		if ($total >= 5500) {
			redirect(base_url('3b4ddb688f47b39dea90c21a6a208093'));
		} else {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($pilih == 1 && empty($alamat_perusahaan)) {
					...
					redirect(base_url(''));
				} elseif ($pilih == 2 && empty($alamat_desa)) {
	...
	{
		$data['pesan'] = 'Kami dengan hormat memohon maaf, pendaftaran program Jalan Sehat BUMN telah ditutup karena kuota pendaftaran yang telah terpenuhi. Kami mengucapkan terima kasih atas ketertarikan Anda untuk bergabung dengan program ini.';
		$this->load->view('limit', $data);
	}
}
